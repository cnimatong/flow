...
const ImportantTasksSection = () => {
  ...
  const getImportantQuests = useQuestStore((state) => state.getImportantQuests);
  ...
  useEffect(() => {
    const filteredImportantQuests = getImportantQuests();
    setImportantTasks(filteredImportantQuests);
  }, [quests]);  

  const handleCompleteTask = (id, importance) => {
    completeQuest(id);  

    // Calculate XP based on task importance
    let xpGained = 0;
  ...
  return (
    <YStack paddingVertical={15}>
      <Text fontSize={12} fontWeight="bold" marginBottom={8}>
        Important Tasks
      </Text>
...
export default ImportantTasksSection;
